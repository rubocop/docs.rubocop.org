...
link:https://github.com/rubocop/rubocop/blob/master/docs/modules/ROOT/nav.adoc[nav.adoc].
...
. In `antora-playbook-dev.yml`, replace the remote URL with your local path and use `HEAD` as the branch:
+
[source,yaml]
----
- url: /path/to/your/local/rubocop
  branches: HEAD
  tags: []
  start_path: docs
----
+
Using `branches: HEAD` lets Antora pick up uncommitted changes from your worktree.
...
. Preview the result by opening `build/site/index.html` in your browser.

NOTE: `antora-playbook-dev.yml` is not checked into version control. It's for local use only.
...
----
$ cd docs.rubocop.org
$ docker run -u $(id -u) -v $PWD:/antora:Z --rm -t antora/antora --cache-dir=./.cache/antora antora-playbook.yml && ./generate-redirects.sh
----
...
NOTE: The site only serves tagged releases -- no master/main branches. The latest tagged
